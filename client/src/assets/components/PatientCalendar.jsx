...
// คอมโพเนนต์สำหรับแสดงเซลล์ของวันในปฏิทิน
const CalendarCell = ({ day, status, star, selectedYear, selectedMonth, userId }) => {
  const navigate = useNavigate();
  
  // คำนวณว่าวันที่นี้เป็นวันในอนาคตหรือไม่ ถ้าเปนก็จะกดไม่ได้
  const today = new Date();
  const cellDate = new Date(selectedYear, selectedMonth, day);
  const isFutureDate = cellDate > today;

  // เมื่อกดที่เซลล์ จะนำทางไปยังหน้าประเมินแพทย์=EvaluatePatient
  const handleClick = () => {
    if (!day || isFutureDate) return; // ถ้าค่า day เป็น null หรือ undefined (เช่น ช่องว่างที่ไม่ได้แสดงวันที่) ถ้าวันที่นั้นเป็นวันในอนาคต (isFutureDate เป็น true) ฟังก์ชันจะ return ออกไป ไม่ให้ผู้ใช้คลิกเพื่อไปยังหน้าอื่นได้
    const formattedDate = `${selectedYear}-${String(selectedMonth + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`; //ให้อยู่ในรูปแบบ "YYYY-MM-DD" ให้ค่าของเดือนและวันมี 2 หลักเสมอ (เช่น "2025-03-05"
    navigate(`/dashboard/eval-doctor/${userId}/${formattedDate}`);
  };

  // กำหนดสีของตัวเลขในปฏิทินตามสถานะ 
  const getTextColor = () => {
  ...
  return (
    <div
      ...
      onClick={handleClick} //ถ้าเป็นวันในอนาคต cursor-not-allowed (ไม่สามารถคลิกได้)
    >
...
// คอมโพเนนต์หลักสำหรับแสดงปฏิทิน
const PatientCalendar = ({ calendarData, userId }) => {
  ...
  // แปลงข้อมูลให้เป็นรูปแบบที่สามารถใช้กับปฏิทินได้
  const transformData = (calendarData, selectedYear, selectedMonth) => {
    ...
    calendarData.forEach(({ created_at, feedback_status, feedback_date, star }) => {
      if (!created_at) return; // ถ้าไม่มีข้อมูลวันที่ ให้ข้ามไป
      const dateKey = created_at?.split("T")[0] || "unknown_date"; // แยกเอาส่วนวันที่จาก timestamp (YYYY-MM-DD)
      statusData[dateKey] = feedback_status !== undefined ? feedback_status : "wait"; // ถ้าไม่มีสถานะ ให้ใช้ค่า "wait"
      if (feedback_date || star) {
        starData[dateKey] = true; // ถ้ามี feedback_date หรือได้รับดาว กำหนดค่าวันนั้นให้มีดาว
      }
    });
    

    // กำหนดค่าเริ่มต้นสำหรับวันที่ไม่มีข้อมูลการประเมิน
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (!(dateKey in statusData) && !(dateKey in starData)) {
        statusData[dateKey] = "notsent"; // notsent=ไม่ได้ส่งประเมิน สีแดง
      }
    }
